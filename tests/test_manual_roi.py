...
class TestMasks:
    ...
    def test_a_tiny_roi_keeps_its_pixels(self, widget):
        ...
        for placement in OUTLINE_PLACEMENTS:
            outline = widget.masks.edges(1, placement)
...
class TestMaskAppearance:
    ...
    def test_line_opacity_is_independent_of_the_fill(self, widget):
        ...
        widget.refresh_overlay()
        alpha = widget.overlay.data.value[..., 3]
        edges = widget.masks.edges(widget.outline_width, widget.outline_placement) > 0
    ...
    def test_color_modes(self, widget):
        ...
        for i in range(3):
            widget.add_roi(square(2 + 14 * i, 2, 9))
        widget.classes.assign([0, 1], 0)  # same group, ROI 2 left unlabeled
        ...
        assert np.allclose(colors[0], widget.classes.color(0))
    ...
    def test_grouped_colors_survive_to_the_overlay(self, widget):
        ...
        assert np.array_equal(first, (np.array(widget.classes.color(0)) * 255).astype(np.uint8))


class TestTraces:
    """The two per-row actions: Quick trace and Extract trace."""

    def test_the_row_has_both_actions_as_icons(self, widget):
        from mbo_utilities.gui.manual_roi import (
            EXTRACT_TRACE_ICON,
            QUICK_TRACE_ICON,
        )

        actions = widget._row_actions()
        assert [a.icon for a in actions] == [QUICK_TRACE_ICON, EXTRACT_TRACE_ICON]
        # icons only, with the name carried by the hover text
        assert all(len(a.icon) <= 2 for a in actions)
        assert actions[0].tooltip.startswith("Quick trace")
        assert actions[1].tooltip.startswith("Extract trace")

    def test_mask_trace_is_the_masked_mean_per_frame(self, widget):
        from mbo_utilities.gui.manual_roi import mask_trace

        widget.add_roi(square(10, 10, 9))
        movie = widget.plane_movie()
        mask = widget.labels == 1
        trace = mask_trace(movie, mask)
        stack = np.stack([np.asarray(movie[t]) for t in range(movie.shape[0])])
        assert trace.shape == (movie.shape[0],)
        assert np.allclose(trace, stack[:, mask].mean(axis=1), atol=1e-5)

    def test_plane_movie_slices_frames_for_batched_extractors(self, widget):
        movie = widget.plane_movie()
        block = movie[slice(1, 4)]
        assert block.shape == (3, 64, 64)
        assert np.allclose(block[0], movie[1])

    def test_quick_trace_averages_the_mask(self, widget):
        widget.add_roi(square(10, 10, 9))
        widget.quick_trace(0)
        entry = drain_trace(widget)
        assert entry["source"] == "quick"
        assert entry["F"].shape == (1, 6)
        movie = widget.plane_movie()
        stack = np.stack([np.asarray(movie[t]) for t in range(6)])
        assert np.allclose(entry["F"][0], stack[:, widget.labels == 1].mean(axis=1), atol=1e-5)

    def test_quick_trace_opens_the_plot_on_that_roi(self, widget):
        widget.add_roi(square(10, 10, 9))
    ...
    def test_actions_are_disabled_without_a_movie(self, widget, monkeypatch):
        monkeypatch.setattr(widget, "plane_movie", lambda: None)
        widget.add_roi(square(10, 10, 9))
    ...
    def test_saving_a_trace_writes_an_npz(self, widget, tmp_path):
        widget.fpath = tmp_path / "movie.tif"
        widget.add_roi(square(10, 10, 9))
        widget.quick_trace(0)
        entry = drain_trace(widget)
        widget.save_trace(0)
        saved = np.load(tmp_path / "roi1_trace.npz")
        assert np.allclose(saved["F"], entry["F"])
        assert str(saved["source"]) == "quick"
...
class TestSorting:
    ...
    def _order(self, widget):
        ...
        widget.classes.assign([0], 1)
        widget.classes.assign([2], 0)
    ...
    def test_sorting_by_area_orders_by_area(self, widget):
        ...
        order.rebuild()
        areas = widget.masks.areas()[order.order]

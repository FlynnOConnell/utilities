...
from fastplotlib.layouts import ImguiFigure as Figure
from fastplotlib.graphics import ImageGraphic
from fastplotlib.utils import calculate_figure_shape, quick_min_max
...
# --- mbo compat aliases (not in the original branch file) --------------------
# surface the mbo-fastplotlib API the GUI was written against
from mbo_utilities.gui._fpl_compat import _NamedIndices as _MboNamedIndices


def _mbo_graphics(self):
    return self.managed_graphics


def _mbo_n_sliders(self):
    return len(self.slider_dims)


def _mbo_sliders_ui(self):
    return self._image_widget_sliders


def _mbo_indices_get(self):
    return _MboNamedIndices(self)
...
ImageWidget.graphics = property(_mbo_graphics)
